...
public class ShieldStatContainer : MonoBehaviour 
{
    ...
    void Update()
    {
        hpLevel = (PlayerPrefs.GetInt("ShdLevel"));
        if(hpLevel == 0)
    ...
    public void LevelUp()
    {
        if(Convert.ToInt32(goldCostText.text) <= goldAmnt)
        {
            ...
            hpLevelTemp = (PlayerPrefs.GetInt("ShdLevel")) + 1;
            PlayerPrefs.SetInt("ShdLevel", hpLevelTemp);
        }
    }
}